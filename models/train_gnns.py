...
#MAKE TWO OF THESE FUNCTIONS
# function 1 does all this but returns list of samples with additional label spurious/non spurious
# function 2 input is that list, changes loss function, returns accuracy/additional metrics 
...
def train_GC_first_pass(model_type):
    ...
    # report test msg
    gnnNets = torch.load(os.path.join(ckpt_dir, f'{model_args.model_name}_{model_type}_{model_args.readout}_best.pth'), weights_only=False) # .to_device()
    gnnNets.to_device()
    gnnNets.eval()
    ...
    return test_state['acc'], annotated_data
...
def test_GC(test_dataloader, gnnNets, criterion):
    ...
    with torch.no_grad():
        for batch_index, batch in enumerate(test_dataloader):
            ...
            loss = criterion(logits, batch.y)
            #FOR EACH SAMPLE, ADD TO SPURIOUS OR NON SPURIOUS BASED ON CLASIFICATION & ACTUAL VALUE
...
if __name__ == '__main__':
    ...
    #run first train func, get spurious set, input into second train func
    accuracy, annotated_data = train_GC_first_pass(model_type)
    print(annotated_data[0])
    print(annotated_data[-1])
    print(annotated_data[1].spurious)
